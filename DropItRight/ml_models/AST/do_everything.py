...
import argparse
import torch
from ml_models.AST.predictor import EffNetPredictor

import argparse
import json

sys.path.append(os.path.join(os.path.dirname(__file__), '..'))  # nopep8
from ml_models.AST.data_utils.seq_dataset import SeqDataset
...
def predict_one_song(predictor, wav_path, song_id, results, do_svs, tomidi, output_path, onset_thres, offset_thres):
    ...
    if tomidi:
        return convert_to_midi(results, song_id, output_path)
...
def vocal_trans(wav_name, device):
    model_path = "ml_models/AST/models/1005_e_4"
    input_path = wav_name
    output_path = "asdf"
    ...
    my_predictor = EffNetPredictor(device=device, model_path=model_path)
    song_id = '1'
    results = {}
    do_svs = False
    ...
    if os.path.isfile(input_path):
        return predict_one_song(my_predictor, input_path, song_id, results, do_svs=do_svs
            , tomidi=True, output_path=output_path, onset_thres=0.4, offset_thres=0.5)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--model_path', default="ml_models/AST/models/1005_e_4", help="model path")
